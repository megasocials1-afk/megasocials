...
export class AuthService {
    static async register(email, username, password, referralCode) {
        ...
        const result = await db.query(`INSERT INTO users (id, email, username, password_hash, referral_code, referred_by, email_verified)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, false)
       RETURNING id, email, username, balance, role`, [email, username, hashed, newReferral, referredBy]);
        if (referredBy) {
            await db.query(`INSERT INTO referrals (referrer_id, referred_id, earnings) VALUES ($1, $2, 0)`, [referredBy, result.rows[0].id]);
        }
        const code = Math.floor(100000 + Math.random() * 900000).toString();
        await db.query('INSERT INTO email_verifications (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'15 minutes\')', [result.rows[0].id, code]);
        await this.sendVerificationEmail(email, code);
        return result.rows[0];
    }
    static async verifyEmail(userId, code) {
        const res = await db.query('SELECT * FROM email_verifications WHERE user_id = $1 AND code = $2 AND expires_at > NOW()', [userId, code]);
        if (!res.rows.length)
    ...
    static async forgotPassword(email) {
        ...
        const code = Math.floor(100000 + Math.random() * 900000).toString();
        await db.query('INSERT INTO password_resets (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'15 minutes\')', [user.rows[0].id, code]);
    ...
    static async resetPassword(email, code, newPassword) {
        ...
        if (!user.rows.length)
            throw new Error('Email not found');
        const res = await db.query('SELECT * FROM password_resets WHERE user_id = $1 AND code = $2 AND expires_at > NOW()', [user.rows[0].id, code]);
        if (!res.rows.length)
            throw new Error('Invalid or expired code');
