...
export class AuthService {
    static async register(email, username, password, referralCode, ip, userAgent) {
        ...
        const result = await query(`INSERT INTO users (id, email, username, password_hash, referral_code, referred_by, email_verified)
       VALUES ($1, $2, $3, $4, $5, $6, false)
       RETURNING id, email, username, balance, role`, [uuidv4(), email, username, hashed, newReferral, referredBy]);
        const user = result.rows[0];
        if (referredBy) {
            await query(`INSERT INTO referrals (referrer_id, referred_id, earnings) VALUES ($1, $2, 0)`, [referredBy, user.id]);
        }
        const code = Math.floor(100000 + Math.random() * 900000).toString();
        await query('INSERT INTO email_verifications (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'15 minutes\')', [user.id, code]);
        console.log('📧 OTP for', email, ':', code);
        await ActivityLogService.log(user.id, 'user_register', { email, username }, ip, userAgent);
        return user;
    }
    static async verifyEmail(userId, code) {
        const res = await query('SELECT * FROM email_verifications WHERE user_id = $1 AND code = $2 AND expires_at > NOW()', [userId, code]);
        if (!res.rows.length)
    ...
    static async forgotPassword(email, ip, userAgent) {
        ...
        const code = Math.floor(100000 + Math.random() * 900000).toString();
        await query('INSERT INTO password_resets (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'15 minutes\')', [user.rows[0].id, code]);
        console.log('🔑 Reset code for', email, ':', code);
        await ActivityLogService.log(user.rows[0].id, 'user_forgot_password', { email }, ip, userAgent);
        return { success: true };
    }
    static async resetPassword(email, code, newPassword, ip, userAgent) {
        const user = await query('SELECT id FROM users WHERE email = $1', [email]);
        if (!user.rows.length)
            throw new Error('Email not found');
        const res = await query('SELECT * FROM password_resets WHERE user_id = $1 AND code = $2 AND expires_at > NOW()', [user.rows[0].id, code]);
        if (!res.rows.length)
