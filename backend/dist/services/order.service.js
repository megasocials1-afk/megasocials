...
export class OrderService {
    static async createOrder(userId, serviceId, link, quantity, promoCode) {
        ...
        try {
            ...
            if (promoCode) {
                const promoRes = await db.query(`SELECT * FROM promo_codes WHERE code = $1 AND status = 'active' AND (expires_at IS NULL OR expires_at > NOW())`, [promoCode]);
    ...
    static async syncOrderStatus(orderId) {
        ...
        await db.query(`UPDATE orders SET status = $1, start_count = $2, current_count = $3, remains = $4, updated_at = NOW() WHERE id = $5`, [newStatus, statusData.startCount, statusData.currentCount, statusData.remains, orderId]);
