...
export default {
  ...
  suspendUser: async (req: any, res: any) => {
    try {
      ...
      await db.query(
        ...
        [newStatus, reason || null, req.user?.id || 'admin', action === 'ban' ? new Date() : null, id]
      );
      res.json({ success: true });
    } catch {
  ...
  createProvider: async (req: any, res: any) => {
    try {
      ...
      const result = await db.query(
        `INSERT INTO providers (id, name, api_url, api_key, api_secret, type, config, status) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, 'active') RETURNING *`,
        [name, api_url, api_key, api_secret, type || 'smmrug', config || {}]
      );
  ...
  getAllOrders: async (req: any, res: any) => {
    try {
      ...
      const result = await db.query(
        `SELECT o.*, u.email, u.username FROM orders o JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC LIMIT $1 OFFSET $2`,
        [limit, offset]
      );
      res.json(result.rows);
    } catch {
  ...
  getActivityLogs: async (req: any, res: any) => {
    try {
      const { limit = 100 } = req.query;
      const result = await db.query(
        `SELECT al.*, u.email FROM activity_logs al LEFT JOIN users u ON al.user_id = u.id ORDER BY al.created_at DESC LIMIT $1`,
        [limit]
      );
      res.json(result.rows);
    } catch {
