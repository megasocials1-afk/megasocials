...
export class ServiceService {
  ...
  static async syncServices(providerId: string) {
    ...
    for (const ps of providerServices) {
      ...
      if (existing.rows.length) {
        await db.query(
          `UPDATE services SET name = $1, description = $2, min = $3, max = $4, rate = $5, cost = $6, profit = $7, provider_service_category = $8, updated_at = NOW() WHERE id = $9`,
        ...
        await db.query(
          ...
          [providerId, ps.id, categoryId, ps.name, ps.description, ps.min, ps.max, ps.rate, cost, cost - ps.rate, ps.category]
        );
      }
    }
  ...
  private static async ensureCategory(categoryName: string) {
    ...
    const insert = await db.query(
      'INSERT INTO categories (id, name) VALUES (gen_random_uuid(), $1) RETURNING id',
      [categoryName]
    );
