...
export class AuthService {
  ...
  private static async sendVerificationEmail(email: string, code: string) {
    const transporter = nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: process.env.SMTP_USER || 'megasocials1@gmail.com',
        pass: process.env.SMTP_PASS || 'zdfkciaqtunvqmbg',
      },
    });
    await transporter.sendMail({
      from: process.env.SMTP_USER || 'megasocials1@gmail.com',
      to: email,
      subject: 'Verify your Mega Socials account',
      html: `<h1>Welcome to Mega Socials!</h1>
