...
export class OrderService {
  static async createOrder(userId: string, serviceId: string, link: string, quantity: number, promoCode?: string) {
    ...
    try {
      ...
      try {
        ...
        await client.query(
          `INSERT INTO orders (id, user_id, service_id, quantity, link, price, cost, profit, status, provider_response, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'pending', $9, NOW(), NOW())`,
          [orderId, userId, serviceId, quantity, link, price, service.cost * quantity, price - (service.cost * quantity), {
  ...
  static async syncOrderStatus(orderId: string) {
    ...
    await query(
      `UPDATE orders SET status = $1, start_count = $2, current_count = $3, remains = $4, updated_at = NOW()
       WHERE id = $5`,
  ...
  static async retryPendingOrders() {
    const res = await query(
      `SELECT id, user_id, service_id, quantity, link, price FROM orders
    ...
    for (const order of res.rows) {
      try {
        ...
        await query(
          `UPDATE orders SET provider_order_id = $1, provider_response = NULL, updated_at = NOW() WHERE id = $2`,
          [providerOrder.orderId, order.id]
        );
  ...
  static async getUserOrders(userId: string, limit: number = 50, offset: number = 0) {
    const res = await query(
  ...
  static async getAllOrders(limit: number = 50, offset: number = 0) {
    const res = await query(
